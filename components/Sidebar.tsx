...
import { LayoutDashboard, Search, Settings, LogOut, ChevronLeft, Menu, Package, Handshake, Bot , Bell} from 'lucide-react';
...
const Sidebar = ({ isCollapsed, setIsCollapsed }: SidebarProps) => {
  ...
  navItems.push({ name: 'Notifications', path: '/notifications', icon: <Bell size={20} /> });
  ...
  return (
      ...
      <button
        ...
        className={`absolute top-8 w-8 h-8 bg-white border border-slate-200/50 shadow-sm rounded-full flex items-center justify-center text-slate-500 hover:text-blue-600 hover:scale-110 transition-all z-50
            ${isCollapsed ? '-right-12 md:-right-4' : '-right-4'}`}
      >
